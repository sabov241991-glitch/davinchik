...
# ===== СТИХ (узкая колонка, как на скрине) =====
POEM = [
...
# Русские месяцы (родительный падеж) и дни недели
RU_MONTHS = {
...
@app.route("/")
def index():
    # Киевское время (автоматически учитывает переходы)
    ...
    # Данные для низа страницы (заглушки)
    context = dict(
...
def login():
    # Логика авторизации пока не нужна — возвращаем на главную
    return redirect(url_for("index"))

# Страница правил регистрации (чтобы ссылка с главной не падала)
@app.route("/reg")
def reg():
