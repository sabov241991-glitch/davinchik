from flask import Flask, render_template, request, redirect, url_for
from datetime import datetime
import pytz

app = Flask(__name__)

# ===== ТЕКСТ СТИХА (правильный вид, как в версии 1.1) =====
POEM = [
"Когда ночами мне одному не",
"спится,",
"К компьютеру душа меня",
"зовет.",
"За окнами на ветке спит",
"синица,",
"А утром соловей там запоет.",
"",
"Письмо твое в Давинчике",
"зависает,",
"С улыбкой на лице его",
"прочту,",
"И каждый смайлик душу",
"согревает,",
"Я в сердце о тебе храню мечту!",
"",
"Рука коснется букв",
"клавиатуры,",
"И в каждом слове нежность и",
"любовь,",
"Пускай, что небеса сегодня",
"хмуры,",
"Я знаю, что напишешь ты мне",
"вновь!",
"",
"Я верю, что ты будешь",
"улыбаться,",
"И верю в виртуальную любовь!!!",
"По-разному мы можем все",
"влюбляться,",
"Но одинаково горит от страсти",
"кровь..."
]

# ==== СТАТИКА ====
HEADER_TITLE = "ДАВИНЧИК"
IN_CHAT = 5
NEWBIES = 0
BIRTHDAYS = 0
REGS_TOTAL = 12
MALES = 6
FEMALES = 6
VISITS_DAY = 0

# ===== МАРШРУТЫ =====
@app.route("/")
def index():
    tz = pytz.timezone("Europe/Kiev")
    now = datetime.now(tz)
    time_str = now.strftime("%H:%M:%S")
    date_str = now.strftime("%d %B %Y")  # дата по-русски
    weekday_str = now.strftime("%A")     # день недели по-русски

    # Переводы месяца и дня
    months = {
        "January": "Января", "February": "Февраля", "March": "Марта",
        "April": "Апреля", "May": "Мая", "June": "Июня",
        "July": "Июля", "August": "Августа", "September": "Сентября",
        "October": "Октября", "November": "Ноября", "December": "Декабря"
    }
    weekdays = {
        "Monday": "Понедельник", "Tuesday": "Вторник",
        "Wednesday": "Среда", "Thursday": "Четверг",
        "Friday": "Пятница", "Saturday": "Суббота",
        "Sunday": "Воскресенье"
    }

    for en, ru in months.items():
        if en in date_str:
            date_str = date_str.replace(en, ru)

    weekday_str = weekdays.get(weekday_str, weekday_str)

    return render_template(
        "index.html",
        header_title=HEADER_TITLE,
        time_str=time_str,
        date_str=date_str,
        weekday_str=weekday_str,
        poem_lines=POEM,
        in_chat=IN_CHAT,
        newbies=NEWBIES,
        birthdays=BIRTHDAYS,
        regs_total=REGS_TOTAL,
        males=MALES,
        females=FEMALES,
        visits_day=VISITS_DAY,
        perf_time="0.0002",
        compression_pct="69.2"
    )

@app.route("/reg/<stage>")
def reg(stage):
    if stage == "rules":
        return render_template("reg_rules.html", header_title=HEADER_TITLE)
    elif stage == "form":
        return render_template("reg_form.html", header_title=HEADER_TITLE)
    return redirect(url_for("index"))

@app.route("/login", methods=["POST"])
def login():
    nick = request.form.get("nick")
    password = request.form.get("password")
    return redirect(url_for("index"))

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000)
