...
def perf_and_compression(rendered_html: str):
    ...
    # имитируем «время генерации» как длину gzip-буфера в секундах/млн для стабильности
    elapsed = len(buf.getvalue()) / 1_000_000
...
# Промежуточная страница: правила регистрации
@app.route("/register")
def register_rules():
...
if __name__ == "__main__":
    # локально можно python main.py; на Render будет запускаться через gunicorn (см. Procfile)
