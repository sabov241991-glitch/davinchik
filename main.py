...
def seed_db():
    ...
    # ID1..11 — системные: 5 парней, 6 девушек; ID12 — Admin (мужчина) => итого 6/6
    for i in range(1, 12):
    ...
    # Отметим «онлайн» нескольких пользователей (UTC)
...
def index():
    ...
    # Фиксируем визит в базу (UTC)
    db.session.add(Visit()); db.session.commit()

    # Киевское время (автоматический переход лето/зима)
    now_local = datetime.now(ZoneInfo("Europe/Kyiv"))

    # Онлайн по UTC: последние 5 минут
    online_window = datetime.utcnow() - timedelta(minutes=5)
    ...
    # Черновой рендер — для расчёта реального сжатия
    draft_html = render_template(
        "index.html",
    ...
    # Реальный расчёт сжатия и времени
    gz_draft = gzip.compress(draft_html.encode("utf-8"))
    ratio = (1 - len(gz_draft)/max(1, len(draft_html.encode("utf-8")))) * 100
    elapsed = perf_counter() - t0

    # Финальный рендер — уже с реальными цифрами
    ...
    # Если браузер поддерживает gzip — реально сжимаем ответ
    accept = request.headers.get("Accept-Encoding", "")
    if "gzip" in accept.lower():
        gz_body = gzip.compress(final_html.encode("utf-8"))
        resp = Response(gz_body, status=200, mimetype="text/html; charset=utf-8")
        resp.headers["Content-Encoding"] = "gzip"
        resp.headers["Vary"] = "Accept-Encoding"
        resp.headers["X-Compression-Ratio"] = f"{ratio:.1f}%"
        return resp

    # Иначе — отдаём как есть (но метрика внизу остаётся реальная)
    resp = Response(final_html, status=200, mimetype="text/html; charset=utf-8")
    resp.headers["Vary"] = "Accept-Encoding"
    resp.headers["X-Compression-Ratio"] = f"{ratio:.1f}%"
    return resp
