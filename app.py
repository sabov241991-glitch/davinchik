...
def init_db():
    ...
    # Инициализируем статистику с 0 посетителей
    stats = conn.execute('SELECT * FROM site_stats WHERE id = 1').fetchone()
    if not stats:
        conn.execute('INSERT INTO site_stats (id, daily_visitors, last_reset) VALUES (1, 0, CURRENT_DATE)')
...
def get_daily_visitors():
    ...
    stats = conn.execute('SELECT * FROM site_stats WHERE id = 1').fetchone()
    if not stats:
        # Создаем новую запись с 0 посетителей
        ...
        if last_reset != today:
            # Новый день - сбрасываем счетчик на 1 (текущий визит)
            daily_visitors = 1
            conn.execute('UPDATE site_stats SET daily_visitors = ?, last_reset = ? WHERE id = 1', 
                        (daily_visitors, today))
        else:
            # Тот же день - увеличиваем счетчик
            daily_visitors += 1
            conn.execute('UPDATE site_stats SET daily_visitors = ? WHERE id = 1', (daily_visitors,))
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            if user['id'] != 12 and bcrypt.checkpw(password.encode('utf-8'), user['password']):
                session['user_id'] = user['id']
                session['username'] = user['username']
                session['is_admin'] = user['is_admin']
                update_user_activity(user['id'])
