...
app.config['DAILY_VISITORS'] = 53  # Базовое значение посетителей
...
def init_db():
    ...
    # Таблица для подсчета посетителей
    conn.execute('''
...
def update_daily_visitors():
    ...
    # Проверяем, не нужно ли сбросить счетчик на новый день
    stats = conn.execute('SELECT * FROM site_stats WHERE id = 1').fetchone()
    if stats and stats['last_reset'] != today.strftime('%Y-%m-%d'):
        # Новый день, сбрасываем счетчик
        conn.execute('UPDATE site_stats SET daily_visitors = 1, last_reset = ? WHERE id = 1', 
...
def get_chat_stats():
    ...
    update_daily_visitors()
    
    # Получаем актуальное количество посетителей
    daily_visitors = conn.execute('SELECT daily_visitors FROM site_stats WHERE id = 1').fetchone()['daily_visitors']
    
    # Реальный подсчет онлайн пользователей
    ...
    # Подсчет именинников на сегодня
    today = date.today()
