...
app.secret_key = 'davinchik-secret-key-2025-' + str(int(time.time()))
...
def init_db():
    ...
    conn.execute('''
        CREATE TABLE IF NOT EXISTS site_stats (
            ...
            daily_visitors INTEGER DEFAULT 53,
            last_reset DATE DEFAULT CURRENT_DATE
        )
    ''')
    
    # Инициализируем статистику
    stats = conn.execute('SELECT * FROM site_stats WHERE id = 1').fetchone()
    if not stats:
        conn.execute('INSERT INTO site_stats (id, daily_visitors) VALUES (1, 53)')
    
    for user_id, user_data in SYSTEM_USERS.items():
        existing = conn.execute('SELECT * FROM users WHERE id = ?', (user_id,)).fetchone()
        
        # Для пользователя Admin используем простой пароль без хеширования
        if user_id == 12:
            password = '123456'  # Пароль в открытом виде для Admin
        else:
...
def update_daily_visitors():
    conn = get_db()
    today = date.today().strftime('%Y-%m-%d')
    
    stats = conn.execute('SELECT * FROM site_stats WHERE id = 1').fetchone()
    if not stats:
        conn.execute('INSERT INTO site_stats (id, daily_visitors, last_reset) VALUES (1, 1, ?)', (today,))
        daily_visitors = 1
    else:
        last_reset = stats['last_reset']
        if last_reset != today:
            conn.execute('UPDATE site_stats SET daily_visitors = 1, last_reset = ? WHERE id = 1', (today,))
            daily_visitors = 1
        else:
            conn.execute('UPDATE site_stats SET daily_visitors = daily_visitors + 1 WHERE id = 1')
            daily_visitors = stats['daily_visitors'] + 1
...
def get_chat_stats():
    ...
    daily_visitors = update_daily_visitors()
...
def login():
    if request.method == 'POST':
        ...
        # Специальная проверка для пользователя Admin
        ...
        if user:
            # Для пользователя Admin пароль уже проверен выше
            if user['id'] == 12:
                flash('Неверный логин или пароль', 'error')
                return redirect(url_for('login'))
            
            # Для остальных пользователей проверяем через bcrypt
...
def autologin():
    ...
    if user_id and password:
        ...
        if user:
            # Для пользователя Admin проверяем пароль без хеширования
            if user['id'] == 12 and password == '123456':
                session['user_id'] = user['id']
                session['username'] = user['username']
                session['is_admin'] = user['is_admin']
                update_user_activity(user['id'])
