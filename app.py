...
# Системные пользователи (ID 1-12) - ВКЛЮЧАЯ ВАШ АККАУНТ!
SYSTEM_USERS = {
...
def init_db():
    ...
    # Получаем реальную статистику (только обычных пользователей, ID > 12)
    stats = conn.execute('''
        SELECT 
            COUNT(*) as total_reg,
...
def get_chat_stats():
    ...
    return {
        'online': 7,
        'new_users': stats['new_users_today'] if stats else 0,
        'birthdays': 27,
        'total_reg': stats['total_reg'] if stats else 0,
        'total_men': stats['total_men'] if stats else 0,
        'total_women': stats['total_women'] if stats else 0,
        'daily_visitors': 53,
        'partners': 7
    }
...
# Важно для Render - обработка ошибок
